    ...
    def generate_report(self):
        ...
        with open(REPORT_PATH, "w") as f:
            ...
            for name in scores.keys():
                ...
                f.write(
                    f"The cluster plot shows the geographical distribution of the clusters for the {name} algorithm. Each color represents a different cluster.\n\n"
                )
                f.write(
                    f"![{name} Cluster Plot]({f'reports/figures/png/{name}.png'})\n\n"
                )
                f.write(f"#### Cluster Counts Plot\n\n")
                f.write(
                    f"The cluster counts plot shows the number of data points in each cluster for the {name} algorithm.\n\n"
                )
                f.write(
                    f"![{name} Cluster Counts Plot]({f'reports/figures/png/{name}_counts.png'})\n\n"
                )
                f.write(f"#### Cluster Pie Chart\n\n")
                f.write(
                    f"The cluster pie chart shows the proportion of data points in each cluster for the {name} algorithm.\n\n"
                )
                f.write(
                    f"![{name} Cluster Pie Chart]({f'reports/figures/png/{name}_pie.png'})\n\n"
                )
